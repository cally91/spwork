...
public class FileService {
	...
	public void uploadFileList(BBsVO bbsVO) {
		...
		for (MultipartFile file : files) {
			...
			try {
				...
				// 한개의 파일정보를 insert 수행
				fDao.insert(FileVO.builder().file_bbs_seq(bbs_seq) // tbl_bbs와 tbl_bbs_file 을 연결하는 key
	...
	public List<FileVO> uploads(MultipartHttpServletRequest files) {
		...
		for (MultipartFile file : fileList) {
			...
			fileVOList.add(
					FileVO.builder().file_origin_name(file.getOriginalFilename()).file_name(this.upLoad(file)).build());
		}
		return fileVOList;
	}
