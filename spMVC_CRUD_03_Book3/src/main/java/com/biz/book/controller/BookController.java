...
public class BookController {
	...
	public String book_input(@ModelAttribute BookVO bookVO, Model model) {
		...
		/*
		 * 데이터를 추가한 후 추가된 새로운 리스트를 보고 싶어서
...
//		model.addAttribute("BODY","BOOK-LIST");
		 ...
		 * 존재한다.
		 * 그래서 그 method()호출하여 같은 일을 수행하려고 했더니
		 * 문제가 발생한다.
		 * 
		 * spring 환경에서는 그럴경우
		 * response 를 통해
		 * 웹 브라우저에게 해당 url을 다시 request 하고록 요청 할수있다
		 * 그 키워드가 response:...이다
		 */
		if (ret > 0) {
		//브라우저야
	 ...
	 * 이때 변수는 거의 대부분 String형이다
	 * String 변수형식으로 매개변수를 지정한다
	 * 
	 * 단, 실제 사용할 변수의 형(type)이 문자열이 아닐경우
	 * 문자열 => 사용할 type으로 변환이 가능하면
	 * 사용할 type으로 변수의 형을 지정할수있다.
	 * 
	 * 이때는 가끔적 @RequestMapping을 선언하여 준다.
	 * 그러면 내부적으로 자동 형변환을 하여 값을 수신할 수있다.
	 * 
	 * 하지만 
	 * 만약 보내는 값이 형변환과 적합하지않으면
	 * 서버는 400오류를 reqponse한다.
	 */
