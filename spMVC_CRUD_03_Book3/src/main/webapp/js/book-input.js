...
//	지금 부터 철저한 문법 규칙을 준수하겠다 라는 선언
// 기본 문법과 런타인 동작을 모두 검사하여
// 실수 에러로 변환시키고
//	변수 사용을 단순화 할 수 있도록 선언하는것

//var num1 = 10
//let num2 = 20
//name ="홍길동" : JS 표준에는 사용할 수 있는 문법이지만 가급적 사용 지양
$(function() {
	...
	$("#btn-save").click(function() {
		...
		let msg= " 도서명: "+ s_title +"\n"
		  ...
		  * 값의 유효성 검사
		  * 1. 입력 값이 있나
		  * 2. 입력 값이 원칙, 규칙이 맞느냐 를 검사하고
		  * 3. 모두 통과 했을 떄만 서버로 데이터로 보낸다.
		  * 
		  * 입력 값을 전송하기전에 유효성 검사
		  * 1. 데이터를 서버로 보내서 유효성 검사를 할 수 도 있지만
		 ...
		 if(isNaN(s_price)){
			...
			// 입력 박스에 감긴 값을 지워라
			 //보통 type="password"인경우 주로사용
			 // $("#b_price").val("")
			
			 
			 //원래 입력된 text를 모두 감싸고
			 // 아무 글자나 입력하면 원래 text를 삭제할수 있도록 해라
			 $("#b_price").select()
			 $("#b_price").focus()
			 return false;
		 }
